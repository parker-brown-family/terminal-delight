...
/// claim in its own payload is what Gate 1 ruled out. The one place the wire
/// is stateful is `watch`, which is a subscription and has to know which
...
mod owning {
    ...
    #[test]
    fn two_windows_on_one_session_share_its_panes() {
        // The accepted v1 behaviour, written down because it was briefly the
        // opposite. A window-level steal was built on 2026-09-10 — a second
        // `window` hello dropped the first window's connection and every pane
        // sink it held — and reversed the same day: no user asked for it, the
        // authorisation boundary is the peer-uid check on the socket, and
        // anything that can open a second window on this session can already
        // do far worse than watch it.
        //
        // So: saying hello costs nobody anything, and what arbitrates a pane
        // is attaching to it. Most recent attach wins that pane; every other
        // pane stays exactly where it was; both windows stay live.
        let scratch = scratch("shared");
        let host = Host::with_shell("test", Some("/bin/cat".into()));
        let first_pane = spawn_guarded(&host).pane;
        let second_pane = spawn_guarded(&host).pane;
        host.persist_to(state_file_in(&scratch));
        let layout = "active = 0\n\n[[tabs]]\n\n[tabs.node.Leaf]\ncwd = \"/tmp\"\n";

        let (_a_client, window_a) = connection(&host);
        assert!(matches!(
            say(&host, &window_a, &hello_of(ClientKind::Window)),
            Reply::Hello { .. }
        ));
        let (mut a_first, server) = UnixStream::pair().expect("pair");
        let a_holds_first = match host.attach(first_pane, server) {
            Outcome::Ok(held) => held,
            Outcome::Err(err) => panic!("{err}"),
        };
        let (_a_second, server) = UnixStream::pair().expect("pair");
        assert!(host.attach(second_pane, server).is_ok());

        // A second window arrives and says what it is. Under the steal this
        // was the moment window A lost everything.
        let (_b_client, window_b) = connection(&host);
        assert!(matches!(
            say(&host, &window_b, &hello_of(ClientKind::Window)),
            Reply::Hello { .. }
        ));
        assert!(
            host.list_panes().iter().all(|p| p.attached),
            "a window saying hello took another window's streams: {:?}",
            host.list_panes()
        );
        assert!(
            matches!(
                say(&host, &window_a, &save_line(layout)),
                Reply::Saved {
                    outcome: Outcome::Ok(_)
                }
            ),
            "a window saying hello cost the other one the session file"
        );

        // B attaches to one pane. That pane, and only that pane, changes hands
        // — and A's stream for it ends, which is the mechanism that lets A
        // find out rather than guess.
        let (_b_first, server) = UnixStream::pair().expect("pair");
        let b_holds_first = match host.attach(first_pane, server) {
            Outcome::Ok(held) => held,
            Outcome::Err(err) => panic!("{err}"),
        };
        assert_ne!(
            a_holds_first.serial, b_holds_first.serial,
            "the second attach reused the first one's claim"
        );
        a_first
            .set_read_timeout(Some(Duration::from_secs(5)))
            .expect("a timeout");
        ...
        assert!(
            closed,
            "the pane changed hands without ending the old stream"
        );

        // Everything else is untouched: the other pane is still A's, both
        // panes are still attended, and both windows can still act.
        assert!(
            host.list_panes().iter().all(|p| p.attached),
            "the second pane lost its stream to an attach on the first: {:?}",
            host.list_panes()
        );
        for (name, conn) in [("the first", &window_a), ("the second", &window_b)] {
            assert!(
                matches!(
                    say(&host, conn, &save_line(layout)),
                    Reply::Saved {
                        outcome: Outcome::Ok(_)
                    }
                ),
                "{name} window was frozen out"
            );
        }
    }
    ...
    fn a_tools_hello_takes_nothing_from_the_window_using_the_session() {
        // Nor does anybody else's: this is the same claim as the shared-panes
        // test from the other end, kept because a probe hellos on every launch
        // and a regression here would show up as every launch stealing.
        let (host, pane) = host_with_cat_pane();
