# Usage, vitals and keepalive — what the fleet costs, and whether it is dying
...
## Keepalive — the cache money

| Feature | What it does | Evidence | Status |
|---|---|---|---|
| **Cache keepalive** | Types into an idle agent's prompt before its prompt cache expires, keeping the session warm | `app/src/keepalive.rs` | Shipped |
| **The measurement** | Across 32,037 billed turns in thirty days on this machine: 87 cold returns, 24.2M tokens re-written, average gap 5.0 hours — about **$242, of which roughly $230 buys nothing** | `keepalive.rs` module doc | measured here |
| **AWAY** | An AWAY campfire in the menu bar marks you gone, so keepalive and notifications can tell idle from absent | `keepalive::{is_away,set_away}`; #295 | Shipped |
...
**Shipped.** The LeanCTX savings plugin ([plugins](04-plugins.md)) is the adjacent
