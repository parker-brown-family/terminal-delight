...
| `TD_SPINE` | **Turns the attention rail OFF.** The rail is on by default: a narrow spine at the right edge, `ctrl+shift+N` opens its queue. Set `0`, `false` or `off` to hide it; any other value (or none) leaves it on | on |
