...
def core_evidence(item: dict, ctx: dict) -> dict | None:
    ...
    if item.get("measured"):
        # version 2: what the bake-off observed when this core was fed recorded programs.
        # Observed behaviour outranks an agent's summary of the API.
        ev["measured_by_bake_off"] = {"observed": item["measured"]}
    if not ev:
        return None
