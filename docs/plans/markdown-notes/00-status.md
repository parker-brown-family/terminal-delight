...
- Parker again, after restarting onto that build: *"I don't see the send to agent with an MD file OR any commentable ness --- I expect that If I hold alt hovering ofver the md file diplay overlay --- that it will show BOXES where I can click to add comments to each element"*. Two things were wrong. The 💬 had never been drawn at all: the Markdown backend read each block's place after rebuilding the column, which clears those places, so every mark came out empty; a press read them after a paint, which is why the earlier pane test (a press) passed. They are now read before the rebuild, and the report counts what was drawn (`buttons`, `boxes`) so a test checks the screen rather than the press. And Alt now does what he described, on a Markdown file and a brief alike: every element that takes a note is outlined, and a press anywhere in one opens its note box.
